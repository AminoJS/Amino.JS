...
describe('how do the enpoints react..', () => {
    ...
    it('when a login api call is recived', () => {
        expect(login).toBe('https://service.narvii.com/api/v1/g/s/auth/login')
    });

    it('when a Call for loading the user has arrvied', () => {
        expect(getMe).toBe('https://service.narvii.com/api/v1/g/s/account');
    });

    it('when a community load api call is recived', () => {
        expect(getComs).toBe('https://service.narvii.com/api/v1/g/s/community/joined?start=0&size=50');
    });

    it('when a upload api call is recived', () => {
        expect(upload).toBe('https://service.narvii.com/api/v1/g/s/media/upload');
    })
